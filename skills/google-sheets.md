...
## Mandatory Post-Write Formatting

After appending or updating tickets, always run the formatting step.

Do not report the workflow as complete until all of the following are applied or explicitly reported as unsupported:

- bold header only on `A1:M1`;
- regular text on all populated data rows;
- frozen row 1;
- filter enabled;
- wrapped text;
- top alignment;
- readable column widths;
- thin, light borders across all populated cells;
- dropdowns on `E`, `F`, `G`, `H`, and `M`;
- white background across all populated data cells;
- conditional formatting only in Priority, Sentiment, and Status.

Formatting must preserve all existing ticket content.

## Mandatory Verification

After writing and formatting, read the sheet again and verify:

- the `Tickets` tab still exists;
- the exact 13 headers remain unchanged;
- no visible column was added, removed, renamed, or reordered;
- all written rows contain 13 values;
- data rows are regular, not bold;
- the header remains bold;
- row 1 is frozen;
- filters are enabled;
- every populated cell has thin, light borders;
- both horizontal and vertical table lines are visible;
- dropdown validation exists in all required columns;
- all populated data cells outside `G`, `H`, and `M` use white background;
- no full-row fill colour remains;
- conditional formatting exists only where required;
- no ticket value changed during formatting;
